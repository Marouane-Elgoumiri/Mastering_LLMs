...
from flashinfer.data.cutlass.python.cutlass_cppgen import shape
from label_studio.data_manager.actions import remove_duplicates
...
class DataProcessor:
    ...
    def remove_duplicates(df: pd.DataFrame, column: str) -> pd.DataFrame:
        return df.remove_duplicates(subset=[column], keep='first')
    
    def remove_outliers(df: pd.DataFrame, series: pd.Series, max_val: int) -> pd.DataFrame:
        return df[series <= max_val].copy() 
